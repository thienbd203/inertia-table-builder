...
const { openFilterPopovers } = storeToRefs(store);
const isPopoverOpen = computed(
    () => openFilterPopovers.value?.[props.activeFilter.field] ?? false,
);
...
const assignTextInputRef = (el: Element | ComponentPublicInstance | null) => {
    if (!el) {
        textInputRef.value = null;
        return;
    }

    if (el instanceof HTMLInputElement) {
        textInputRef.value = el;
        return;
    }

    const component = el as ComponentPublicInstance<{ $el?: Element }>;
    const root = component?.$el;

    if (root instanceof HTMLInputElement) {
        textInputRef.value = root;
        return;
    }
...
watch(
    ...
    async (isOpen) => {
        if (
            isOpen === true &&
            filterDef.value.type !== 'select' &&
            filterDef.value.type !== 'date' &&
            filterDef.value.type !== 'boolean'
        ) {
            await nextTick();
            if (import.meta.env.DEV) {
                console.log('[FilterInput] focusing', props.activeFilter.field);
            }
        ...
        router.get(
            ...
            { [key]: val },
            {
                ...
                onSuccess: (page) => {
                    ...
                    const options = props.data.filters.opt?.find(
                        (f) => f.field === filterDef.value.field,
                    )?.options;
                    if (options) {
...
<template>
    ...
    <template v-else-if="filterDef.type === 'custom'">
        <component
            v-if="
                filterDef.component &&
                customFilterComponents[filterDef.component]
            "
            :is="customFilterComponents[filterDef.component]"
            :value="props.activeFilter.value"
            :filterDef="filterDef"
            :activeFilter="props.activeFilter"
            @change="(val: any) => handleSelectChange(val)"
        />
    ...
    <template v-else-if="filterDef.type === 'select'">
        ...
        <template v-if="filterDef.searchable">
            ...
            <Popover
                v-if="
                    filterDef.multiple ||
                    props.activeFilter.operator === 'in' ||
                    props.activeFilter.operator === 'notIn'
                "
            >
                <PopoverTrigger asChild>
                    <Button
                        variant="outline"
                        class="w-full justify-between bg-popover"
                        role="combobox"
                    >
                        {{
                ...
                <PopoverContent
                    class="max-h-64 w-[var(--radix-popover-trigger-width)] overflow-y-auto p-0"
                >
                    <Command>
                        <CommandInput
                            placeholder="Search..."
                        ...
                        <CommandEmpty
                            ...
                            "
                            >No option found.</CommandEmpty
                        >
                        <CommandGroup>
                            <CommandItem
                                v-for="opt in filterDef.options"
                                :key="opt.id"
                                :value="String(opt.value)"
                                @select="
                                    () => handleCommandMultiSelect(opt.value)
                                "
                            >
                                <Checkbox
                                    class="mr-2"
                                    :model-value="
                                        localValue.includes(opt.value)
                                    "
                                    @update:modelValue="
                                        (checked) =>
        ...
        <template v-else>
            ...
            >
                ...
                <PopoverContent
                    class="max-h-64 w-[var(--radix-popover-trigger-width)] overflow-y-auto p-2"
                >
                    <label
                        v-for="opt in filterDef.options"
                        :key="opt.id"
                    ...
                    >
                        <Checkbox
                            :model-value="localValue.includes(opt.value)"
                            @update:modelValue="
                                (checked) => toggleOption(opt.value, checked)
                            "
                        />
                        <span>{{ opt.label }}</span>
                    </label>
